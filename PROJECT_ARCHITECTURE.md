...
### 6.4 Hesap Silme / KVKK Unutulma Hakkı (Account Deletion)

**Model: anonimleştirme, hard-delete değil.** `DELETE /api/account` (mevcut şifre zorunlu, Discord-tarzı re-auth — diğer hassas hesap değişiklikleri gibi), `Backend.Accounts.delete_account/2`'de tek bir `Ecto.Multi` transaction'ı olarak uygulanır: hepsi ya da hiçbiri. Kullanıcının `users` satırı **silinmez** — `username`/`email`/`password_hash` rastgele, tahmin edilemez değerlere çevrilir ve yeni bir `deleted_at` (utc_datetime, nullable) kolonu `now()`'a set edilir. `deleted_at`, "bu hesap silindi mi" sorusunun tek doğruluk kaynağıdır: `Backend.Accounts.display_username/1` bunu kontrol edip UI'da gerçek (anonim) username yerine `"[silinmiş kullanıcı]"` döner — mesaj/DM yazarlığı, reaction'lar ve DM oda geçmişi bu yüzden hiç dokunulmadan kalabilir, sadece görüntülenen isim değişir.

**Veri haritası — hangi veri ne oluyor:**

| Veri | Aksiyon | Gerekçe |
|---|---|---|
| `users` (silinen hesabın kendi satırı) | **Anonimleştir** — `username`→`deleted_user_<12hex>`, `email`→`deleted-<12hex>@deleted.invalid`, `password_hash`→rastgele (asla bilinemeyecek bir değere), `deleted_at`→now(), `token_version`→+1 | Satır silinmez ki mesaj/DM/reaction FK'leri hiç kırılmasın; `deleted_at` tek doğruluk kaynağı olarak `display_username/1` üzerinden UI'a yansır |
| Chat mesajları / DM mesajları (`messages`, `dm_messages`) | **Korunur** (içerik dokunulmaz) | Yazarın adı `display_username/1` ile `"[silinmiş kullanıcı]"` gösterilir — `chat_channel.ex`, `dm_channel.ex`, `dm_controller.ex`, `channel_controller.ex`'in mesaj serializer'ları bunu kullanır |
| DM odaları (`dm_rooms`) | **Korunur** | Kullanıcı satırı yaşadığı için FK kırılmaz; karşı taraf odayı DM listesinde görmeye devam eder (`DirectMessages.to_view/2` de `display_username/1` kullanır) |
| Reaction'lar (`message_reactions`, `dm_message_reactions`) | **Korunur** | Satır silinmez, sadece reactor'ın kimliği (gösterilirse) anonim olur |
| Arkadaşlık ilişkileri + engellemeler (`friendships`, tüm statüler, iki yönde) | **Hard-delete** | Karşı tarafın listesinden kaybolur, bekleyen istekler iptal olur |
| Sunucu üyelikleri (`server_members`, sahibi olmadığı sunucular) | **Hard-delete** | Hesap silinince kullanıcı tüm sunuculardan otomatik çıkar |
| Sunucu sahiplikleri (`servers.owner_id == user`) | **Koşullu** — başka üye varsa **en eski üyeye** (`ServerMember.inserted_at asc`) devret + eski sahibin kendi üyelik satırı da silinir; başka üye yoksa `Repo.delete(server)` çağrılır | DB'nin kendi `on_delete: :delete_all` zinciri (`servers→channels→messages→message_reactions`, `servers→server_members`, `servers→server_invites`) sunucuyu tüm bağımlı verisiyle temizler |
| S3/local upload dosyaları | **Dokunulmaz — silinecek bir dosya yok** | Projede kullanıcıya özel bir profil resmi/avatar alanı hiç yok; mesaj ekleri (`file_url`) mesaj içeriğinin parçası ve zaten silinmiyor |
| Presence kayıtları | **Dokunulmaz, kendiliğinden temizlenir** | `Backend.Presence` tamamen bellek içi (`Phoenix.Presence`/ETS) — DB'de hiç satır yok; `token_version` bump + soket kopmasıyla anında temizlenir |
| Server invites (kullanıcının başkasının sunucusunda oluşturduğu davetler) | **Dokunulmaz** | Kullanıcı satırı silinmediği için FK kırılmaz, kod kapsam dışı bırakıldı |

**Atomiklik ve sıralama (`Backend.Accounts.delete_account/2` → `build_deletion_multi/1`):** `Ecto.Multi` adımları — `:owned_servers` (oku) → `:server_outcomes` (devret/sil) → `:remaining_memberships` (oku) → `:remove_memberships` (sil) → `:friendship_rows` (oku) → `:remove_friendships` (sil) → `:anonymized_user` (`User.deletion_changeset/1`) — hepsi **tek bir DB transaction'ında**. Bildirimler (`server_deleted`, `member_left`, `server_updated`, `friend_removed`) **sadece transaction commit olduktan sonra** gönderilir — bir broadcast geri alınamaz, bu yüzden Multi'nin ortasında hiçbir yayın yapılmaz. `backend/test/backend/accounts_test.exs`'teki atomiklik testi, DB'ye doğrudan (changeset'i bypass ederek) boş bir `servers.name` yazıp gerçek bir `validate_required(:name)` hatasını transfer adımında tetikler ve transaction'ın TAMAMEN geri alındığını (başka, sağlıklı bir sunucunun devri bile olsa) kanıtlar.

**`ServerMember.inserted_at` hassasiyeti `utc_datetime_usec`'e genişletildi** (`widen_server_members_timestamps_precision` migration) — "en eski üye" sıralaması artık bu alana dayandığından, saniye-hassasiyetinin (aynı saniyede katılan iki üyenin eşit karşılaştırılması) yol açacağı belirsiz sıralamayı önlemek için; `messages`/`dm_messages`/`users.token_valid_from`'a daha önce uygulanan aynı düzeltme.

**Dialyzer notu:** `build_deletion_multi/1` üzerinde `@dialyzer {:no_opaque, build_deletion_multi: 1}` var — `Ecto.Multi` bu projede ilk kez kullanılıyor (bkz. [Bölüm 7](#7-bilinen-boşluklar--devam-notları) madde 9) ve bu ilk kullanım, `%Ecto.Multi{}`'nin `:names` alanındaki `MapSet`'in opak iç temsiliyle ilgili, projenin tam Elixir/OTP çiftine (Erlang/OTP 29, Elixir 1.20) özgü, Ecto'nun kendi derlenmiş koduna ait bilinen bir Dialyzer yanlış-pozitifini ortaya çıkarıyor — `Multi.new()`'ın `Multi.run/3`'e aktarılmasında `call_without_opaque` uyarısı veriyor. `Backend.AccountsTest`'in tam test seti (atomiklik testi dahil) bunun kod seviyesinde zararsız olduğunu kanıtlıyor.

**Frontend:** `UserSettingsModal`'ın "Hesap Silme / Veri Yönetimi" kategorisi artık dolu (önceden bu kategori projedeki **son** "Yakında" placeholder'ıydı — `PlaceholderSettings` component'i bu turda tamamen kaldırıldı, çünkü 7 kategorinin de dolduğu artık hiç ulaşılamıyordu). Üç katmanlı onay: mevcut şifre + "Bu işlemin geri alınamaz olduğunu anlıyorum" checkbox'ı + tam olarak `HESABIMI SİL` yazılması gereken bir metin kutusu — üçü de dolmadan buton disabled kalır. Başarılı silme, `PasswordForm`'un "diğer cihazlarda oturum kapat" akışıyla aynı deseni izler: bir başarı mesajı gösterilir, ardından `disconnectSocket()` + `forceLogout()` ile login ekranına yönlendirilir.

**Doğrulama:** `mix precommit` (Credo/Dialyzer/audit temiz, 193 test — atomiklik, sunucu devri/silme, arkadaşlık/engelleme temizliği, token geçersizleşme, `display_username` dahil) ve `npm run build` + `npm run test` (129 test) çalıştırıldı, hepsi yeşil. Tarayıcı eklentisi bağlantı sorunu nedeniyle uçtan uca doğrulama ilk turda tamamlanamamıştı; bir sonraki turda tek kullanımlık test hesaplarıyla (`ard` hesabına dokunulmadan) tamamlandı:

- **Senaryo A (tek başına sahip → sunucu silinir):** gerçek tarayıcıda uçtan uca manuel olarak doğrulandı — hesap silindikten sonra eski kimlik bilgileriyle giriş başarısız oldu, ve sunucunun (tek üyesi kendisiydi) davet koduyla başka bir hesaptan katılma denemesi de başarısız oldu (sunucu kanallarıyla birlikte tamamen silinmişti).
- **Senaryo B (başka üye var → sahiplik devredilir):** backend test seviyesinde kanıtlandı — `backend/test/backend/accounts_test.exs`'teki `"delete_account/2 transfers ownership to the oldest other member when other members exist"` testi, `updated_server.owner_id == first_member.id` (yeni sahip doğru kişi), `refute Servers.member?(server.id, owner.id)` (eski sahip üyelikten çıkarıldı) ve yeni sahibin kendi `ServerMember` satırındaki `role == "owner"` alanını ayrı ayrı doğruluyor; izole çalıştırılıp geçtiği teyit edildi. **Bilinçli olarak eksik bırakılan tek parça:** bu senaryonun UI render tarafı (Sunucu Ayarları → Üyeler listesinde "Sahip" rozetinin yeni sahibin adının yanında görsel olarak doğru göründüğü) gerçek tarayıcıda son kez manuel teyit edilmedi — zaman/kapsam kararı olarak buraya not düşülüyor, backend davranışı test seviyesinde kanıtlı olduğu için düşük risk görülüyor.

### 6.5 Arkadaşlar Ekranının Ayrılması + Maskot Entegrasyonu

**Arkadaşlar artık Home'un varsayılan görünümü değil, kendi ayrı ekranı:** Önceden `Chat.tsx`'te `activeServerId` ve `activeDmRoomId` ikisi de `null` olduğunda (Home/DM ikonu aktifken hiçbir DM seçili değilken) ana içerik alanı doğrudan `FriendsPanel`'i gösteriyordu. Artık `ServerSidebar.tsx`'e Home ikonunun hemen altına ayrı bir "Arkadaşlar" ikonu (`lucide-react`'ten `Users`) eklendi; `Chat.tsx`'teki yeni `friendsViewOpen` local state'i (`useState`, `ServerSidebar`'a `friendsActive`/`onSelectFriends`/`onNavigate` prop'larıyla aktarılıyor) main alanının render sırasında en yüksek önceliğe sahip — `friendsViewOpen ? <FriendsPanel/> : activeServerId ? ... : activeDmRoomId ? <DMChatView/> : <EmptyState/>`. Friends ikonuna tıklamak `activeServerId`'yi de `null`'a çeker (bir sunucu kanalı açıkken Friends'e geçilirse sol sidebar da DM listesine dönsün diye); Home ikonuna veya herhangi bir sunucu ikonuna tıklamak (`onNavigate` prop'u üzerinden) `friendsViewOpen`'ı kapatır. Home ekranının kendisi (hiç DM seçili değilken) artık `EmptyState` gösteriyor — Arkadaşlar yönetimi tamamen ayrı bir ekran.

**Ortak `EmptyState` component'i** (`frontend/src/components/EmptyState.tsx` + `.css`) — `zircle-mascot.svg`'yi ortada, altında soluk (`--text-muted`) bir mesaj gösteriyor. İki yerde kullanılıyor: (1) Home/DM ekranında hiçbir DM odası seçili değilken, (2) bir sunucu içinde `activeChannelId` `null` iken (önceden bu durum sessizce boş bir header'la render oluyordu — kategori-only bir sunucuda hiç metin kanalı yoksa fark edilirdi; artık aynı ortak component'le tutarlı bir boş durum gösteriyor). Kod tekrarını önlemek dışında bir amacı yok, mesaj metni prop olarak geçiliyor.

**Maskot, giriş/kayıt sayfasında da kullanılıyor:** `Auth.tsx`'in kartının en üstüne `zircle-mascot.svg` eklendi (`.auth-mascot`, 96px genişlik).

**Favicon kararı:** Proje zaten bir `.ico` değil, `frontend/public/favicon.svg` kullanıyor (index.html'de `<link rel="icon" type="image/svg+xml">`) — bu yüzden SVG→ICO dönüşümü hiç gerekmedi, `favicon.svg`'nin içeriği doğrudan `zircle-mascot.svg` ile aynı path'lere (aynı viewBox `0 0 680 460`, sadece `width`/`height` favicon boyutuna küçültülmüş: 48×32) güncellendi.
...
| 9 | ~~`Ecto.Multi` projede hiç kullanılmıyor~~ **İlk kullanımı eklendi** | `backend/lib/backend/accounts.ex` `build_deletion_multi/1` | `Backend.Accounts.delete_account/2` (bkz. [6.4](#64-hesap-silme--kvkk-unutulma-hakkı-account-deletion)) artık adlandırılmış, geri-izlenebilir adımlarla `Ecto.Multi` kullanıyor. Diğer çoklu-adım DB işlemleri (friendship race condition, channel position bulk update) hâlâ `Repo.transaction/1` + manuel `Repo.rollback/1` ile yapılıyor — bu ilk kullanım onları geriye dönük değiştirmedi, sadece yeni kodun kendi ihtiyacı için seçildi. Bu ilk kullanım, `%Ecto.Multi{}`'nin `:names` alanındaki `MapSet`'in opak temsiliyle ilgili bilinen bir Dialyzer yanlış-pozitifini de ortaya çıkardı — `@dialyzer {:no_opaque, ...}` ile belgelenerek bastırıldı |
...
| 17 | İki faktörlü kimlik doğrulama (2FA) ve aktif oturum/cihaz listesi hiç yok | `backend/lib/backend/accounts.ex`, `backend/lib/backend/accounts/user.ex` | **Bilinçli karar, boşluk değil.** `token_version` (`accounts/user.ex:14`) tek bir global tam sayı sayaçtır — `bump_token_version/1` (`accounts.ex:106-120`) onu `Repo.update_all` ile atomik olarak artırıp o kullanıcının **daha önce verilmiş tüm token'larını** tek seferde geçersiz kılar (parola değişiminde olduğu gibi); ama hangi token'ın hangi cihaz/tarayıcıya ait olduğuna dair hiçbir kayıt tutulmaz, dolayısıyla "aktif oturumlarım" listesi veya "sadece şu cihazdan çıkış yap" gibi seçici bir iptal şu anki modelle mümkün değil — böyle bir özellik, her token'ı bir cihaz/oturum satırına bağlayan ayrı bir tabloyu gerektirir. 2FA'nın da (TOTP/e-posta kodu vb.) hiçbir altyapısı yok. Gerekçe [Bölüm 16]'dakiyle aynı: bu proje şu an gerçek bir kullanıcı tabanını korumuyor (bkz. aşağıdaki Faz 4 notu), ve bu ikisi (per-cihaz oturum takibi + 2FA) mevcut `token_version` modelini yeniden tasarlamayı gerektirecek kadar büyük bir iş — gerçek/değerli hesaplar var olmadan bu yatırımı yapmak yersiz. Gerçek kullanıcı tabanı oluşursa bu madde öncelikli ele alınmalı. |

**Not — Faz 4 (KVKK/gizlilik), "unutulma hakkı" hariç bilinçli olarak ertelendi:** Bu proje şu an kişisel/portfolyo amaçlı kullanılmaktadır, gerçek kullanıcı verisi işlenmesi planlanmadığı için resmi KVKK uyum sürecinin geri kalanı (gizlilik politikası metni, VERBİS kaydı vb.) bilinçli olarak ertelenmiştir. "Unutulma hakkı" kısmı artık kısmen karşılanıyor — bkz. [6.4](#64-hesap-silme--kvkk-unutulma-hakkı-account-deletion), `DELETE /api/account`. Gerçek kullanıcı tabanı oluşursa geri kalan maddeler öncelikli olarak ele alınmalıdır.
